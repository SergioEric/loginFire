...
export class AppComponent {
  ...
  crearUsuarioPorEmail(){
    ...
    this.fireAuth.auth.createUserWithEmailAndPassword(this.model.email, this.model.password).then(res=>{
  ...
  resetarContrasena(){
    ...
    this.fireAuth.auth.sendPasswordResetEmail(this.model.email)
		.then(()=>{
			//email sent
			this.resetInfo =`peticion de contrasena enviada, habre tu email: ${this.model.email}`
		})
		.catch(error=>{
			//error ocurrido
  ...
  loguear(){
    this.fireAuth.auth.signInWithEmailAndPassword(this.model.email, this.model.password).then(res=>{
